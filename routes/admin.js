...
router.get('/profit', (req, res) => {
  return res.status(200).json({
    "Profit": fakeRestaurant.getProfit()
  });
});
...
router.put('/orders/:id', (req, res) => {
  ...
  if (req.body.status === undefined || (req.body.status !== 'closed' && req.body.status !== 'ready')) {
    ...
    return res.status(200).json({
      message: 'Order with ID #' + req.params.id + ' has been change the status to ' + req.body.status
    });
  } else {
    return res.status(404).json({
      message: 'Error! Order with ID #' + req.params.id + ' not found.'
    });
  }
});
